...
void init_video_fifo() {
	video_fifo = (Video_Buffer*) malloc(sizeof(Video_Buffer));
...
int network_thread(void *configuration) {
	...
	while (true) {
		Video_Frame *frame = (Video_Frame*) malloc(sizeof(Video_Frame));
		...
		Video_Frame_Element *element = (Video_Frame_Element*) malloc(sizeof(Video_Frame_Element));
		element->frame = NULL;
		element->next = NULL;
		element->frame = frame;
